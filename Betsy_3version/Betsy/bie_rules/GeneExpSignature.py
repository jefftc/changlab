...
GenesetAnalysis = DataType('GenesetAnalysis', AttributeDef(
    "contents", Database.CONTENTS, 'unspecified', 'unspecified',
    help="contents"), AttributeDef("allgenes", ['yes', 'no'], "no", "no",
                                   help="analyze all geneset"),
                           AttributeDef("automatch", ['yes', 'no'], "no", "no",
                                        help="automatch in geneset analysis"),
                           help="Geneset analysis file")
GenesetPlot = DataType('GenesetPlot', AttributeDef(
    "contents", Database.CONTENTS, 'unspecified', 'unspecified',
    help="contents"), AttributeDef("allgenes", ['yes', 'no'], "no", "no",
                                   help="analyze all geneset"),
                       AttributeDef("automatch", ['yes', 'no'], "no", "no",
...
SignatureScore = DataType('SignatureScore', AttributeDef(
    "contents", Database.CONTENTS, 'unspecified', 'unspecified',
...
GenesetReportFile = DataType('GenesetReportFile',
...
ScoreComparePlot = DataType('ScoreComparePlot',
...
all_modules = [
    Module('score_pathway_with_geneset', [
        GenesetFile, GeneExpProcessing.SignalFile
    ], GenesetAnalysis, OptionDef("geneset_value",
                                  help="geneset to score pathway"),
           #Constraint("quantile_norm",MUST_BE,'yes',1),
           Constraint("gene_center", MUST_BE, 'mean',
                      1), Constraint("gene_normalize", MUST_BE, 'variance', 1),
    ...
    Module('score_pathway_with_geneset_all', [GenesetFile,
                                              GeneExpProcessing.SignalFile],
           GenesetAnalysis,  #Constraint("quantile_norm",MUST_BE,'yes',1),
           Constraint("gene_center", MUST_BE, 'mean',
                      1), Constraint("gene_normalize", MUST_BE, 'variance', 1),
           Constraint("annotate", MUST_BE, 'yes', 1),
           Constraint("unique_genes", MUST_BE, 'high_var', 1),
    ...
    Module('plot_geneset_score_bar', GenesetAnalysis, GenesetPlot,
    ...
    Module('score_pathway_with_scoresig_affymetrix', [
        GeneExpProcessing.SignalFile, GeneExpProcessing.SignalFile
    ], SignatureScore, Constraint("format", MUST_BE, 'tdf', 0),
    ...
    Module('score_pathway_with_scoresig', GeneExpProcessing.SignalFile,
           SignatureScore, Constraint("format", MUST_BE, 'tdf'),
           Constraint("preprocess", CAN_BE_ANY_OF,
                      GeneExpProcessing.PREPROCESS_WOrma, 0),
    ...
    Module(
        'convert_SignatureScore_preprocess', SignatureScore, SignatureScore,
    ...
    Module('make_score_signature_report', [GenesetAnalysis, GenesetPlot,
                                           SignatureScore], GenesetReportFile,
           help="make signature report"),
    Module('compare_signature_predictions', [SignatureScore, SignatureScore,
                                             SignatureScore],
           ScoreCompareReportFile,
    ...
    Module('plot_signature_predictions_comparison', ScoreCompareReportFile,
           ScoreComparePlot,
