...
IntensityPlot = DataType(
    'IntensityPlot',
    Attribute("pca_gene_num",["no", "yes"], "no","no"),
    Attribute(
        "format",[ "tdf", "gct"],"tdf","tdf"),
    # Properties of the data.
    Attribute(
...
ActbPlot = DataType(
    'ActbPlot',
    Attribute("preprocess",["unknown", "agilent", "mas5", "rma",
                            "loess",'illumina'],'unknown','unknown'),
    )
Hyb_barPlot = DataType(
    'Hyb_barPlot')
ControlPlot = DataType(
    'ControlPlot',
    Attribute("preprocess",["unknown", "agilent", "mas5", "rma", "loess"],
          'unknown','unknown'))
BiotinPlot = DataType(
    'BiotinPlot')
...
all_modules = [
   ...
   Module(
    ...
    Module(
        ...
        Consequence("platform",SAME_AS_CONSTRAINT),
        Consequence("duplicate_probe",SAME_AS_CONSTRAINT),
        Consequence("unique_genes",SET_TO_ONE_OF,['average_genes', 'high_var', 'first_gene'])),
      
    Module(
         'select_first_n_genes',
        SignalFile2,SignalFile2,
        UserInput("num_features_value",500),
        Constraint("format",MUST_BE,"tdf"),
        Constraint("logged",MUST_BE,"yes"),
        Constraint("num_features",MUST_BE,"no"),
        Constraint("platform",MUST_BE,"no"),
   ...
   Module( 
    ...
    'plot_intensity_boxplot',
     ...
     Constraint("preprocess",CAN_BE_ANY_OF,["unknown", "illumina", "agilent", "mas5", "rma", "loess"]),
    ...
    Module(
        ...
        Constraint('preprocess',CAN_BE_ANY_OF,["unknown", "agilent","illumina",
                                "mas5", "rma", "loess"]),
        Constraint("format",MUST_BE,"tdf"),
        Constraint("logged",MUST_BE,"yes"),
        Constraint("missing_values",MUST_BE,"no"),
