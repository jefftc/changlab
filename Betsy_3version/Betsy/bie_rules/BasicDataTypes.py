...
ExpressionFiles = DataType("ExpressionFiles", AttributeDef("contents", Database.CONTENTS, 'unspecified', 'unspecified', help="contents"), AttributeDef("filetype", ['unknown', 'cel', 'gpr', 'idat', 'agilent', 'matrix'], 'unknown', 'unknown', help="filetype"), help="Expression file folder, can be CELFiles, IDATFiles,"\
...
GeneListFile = DataType(
    ...
    AttributeDef('cn_filter_data', ['yes', 'no'], 'no', 'no',
                 help="class neighbors filter data or not"), AttributeDef(
                     'gene_order',
                     ['no', "gene_list", "class_neighbors", "t_test_p",
                      "t_test_fdr", 'diff_ttest', 'diff_sam', 'diff_ebayes',
                      'diff_fold_change'], 't_test_p', "t_test_p",
...
all_modules = [
    Module(
        ...
        OptionDef("GSEID",
                  help="GSEID to download"),
        OptionDef("GPLID", "",
                  help="GPDID to download"),
    ...
    Module(
        "download_geo_seriesmatrix", Database.GEOSeries, Database.MatrixFile,
        OptionDef("GSEID",
                  help="GSEID to download"),
        OptionDef("GPLID", "",
                  help="GPDID to download"),
        ...
        Consequence("contents", SAME_AS_CONSTRAINT),
        help="download GEO data from geo website according to GSEID and GPLID"),
    Module('convert_family_soft_to_rename', Database.GEOfamily, RenameFile,
           OptionDef("GSEID",
    ...
    Module("check_geo_file_type", [ExpressionFiles,
                                   Database.MatrixFile], ExpressionFiles,
           Constraint("contents", CAN_BE_ANY_OF, Database.CONTENTS),
           Constraint("contents", SAME_AS, 0, 1),
           Consequence("contents", SAME_AS_CONSTRAINT, 0),
           Constraint("filetype", MUST_BE, 'unknown', 0),
           Consequence("filetype", BASED_ON_DATA, ['matrix', 'cel', 'gpr',
                                                   'idat', 'agilent']),
