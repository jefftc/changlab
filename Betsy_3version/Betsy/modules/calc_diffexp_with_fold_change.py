#calc_diffexp_with_fold_change.py
...
def run(in_nodes, parameters, user_input, network):
    ...
    cmd = ['python', diffexp_bin, data_node.identifier,'--cls_file',
           cls_node.identifier, '--algorithm','fold_change']
    ...
    assert module_utils.exists_nz(outfile), (
        'the output file %s for calc_diffexp_with_fold_change fails' % outfile)
...
def name_outfile(in_nodes,user_input):
    ...
    filename = 'fold_change_' + original_file + '.txt'
