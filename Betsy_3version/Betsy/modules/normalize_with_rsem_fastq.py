...
def preprocess_single_sample(folder,sample,files,out_file,ref):
    ...
    if len(files)==1:
        ...
        command = ['rsem-calculate-expression','--paired-end',input_file1,input_file2,
...
def run(in_nodes,parameters,user_input,network,num_cores):
    ...
    assert 'fastq_ref' in user_input,'should provide ref in human or mouse'
    preprocess_multiple_sample(data_node.identifier, group_dict, outfile, user_input['fastq_ref'])
    assert module_utils.exists_nz(outfile), (
        'the output file %s for normalize_with_rsem_fastq does not exist'
        % outfile)
