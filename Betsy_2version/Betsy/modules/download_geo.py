...
def run(data_node, parameters):
    ...
    GPLID = data_node.attributes['GPLID']
    assert GSEID.startswith('GSE'), 'GSEID %s is not correct' % GSEID
    if GPLID == '___BETSY_ANYATOM___':
    ...
    out_node = bie.Data(rulebase.ExpressionFiles,**new_parameters)
    return out_node


def name_outfile(data_node):
    original_file = module_utils.get_inputid(
        data_node.attributes['GSEID'])
...
def make_unique_hash(data_node,pipeline,parameters):
    GPLID = ''
    if  not data_node.attributes['GPLID'] == '___BETSY_ANYATOM___':
        GPLID = data_node.attributes['GPLID']
    identifier = data_node.attributes['GSEID'] + GPLID
    return identifier
