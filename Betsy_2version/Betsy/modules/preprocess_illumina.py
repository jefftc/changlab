...
def run(data_node, parameters, network):
    ...
    gp_parameters = dict()
    if zipfile.is_zipfile(data_node.attributes['filename']):
        gp_parameters['idat.zip'] = data_node.attributes['filename']
    else:
        zipfile_name = os.path.split(data_node.attributes['filename'])[-1]+'.zip'
        zip_directory(data_node.attributes['filename'],zipfile_name)
    ...
    out_node = bie.Data(rulebase.ILLUFolder,**new_parameters)
    return out_node
