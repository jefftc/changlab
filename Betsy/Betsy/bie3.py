...
def _can_path_produce_data_node(
    network, user_attributes, path, node_id, nodeid2previds):
    x = nodeid2previds.get(node_id, [])
    x = [x for x in x if x in path]
    module_ids = x
    assert module_ids
    for module_id in module_ids:
        ...
        x = [x for x in x if x in path]
        prev_ids = x
        if _get_valid_input_combinations(
            network, module_id, prev_ids, user_attributes,
            node2previds=nodeid2previds, all_output_ids=[node_id]):
            return True
    return False
...
##         # If this node is one of these datatypes, then this can be an input.
##         found = False
##         for i, ids in enumerate(node2ids):
##             if node_id not in ids:
##                 continue
##             yield [node_id], [(node_id, i)], []
##             found = True
##         if not prev_ids and not found:
##             # If this is an start node, then this is a missing input.
##             yield [node_id], [], [node_id]
##         combos = []
##         for prev_id in prev_ids:
...
##     for combo in combos:
##         # Each branch is a generator to this recursive function.
##         assert node_id not in combo
##         branch2info = [
##             _find_paths_by_datanodes_h(
##                 network, user_attributes, x, data_nodes, node2ids)
##             for x in combo]
##         # Try different combinations of paths for each branch.
##         for x in itertools.product(*branch2info):
##             # Merge the information from each branch.
...
##                 path.extend(p)
##                 used_ids.extend(uids)
##                 missing_ids.extend(mids)
##             assert node_id not in path
##             # May have duplicates since different branches may merge
##             # upstream.
##             path = {}.fromkeys(path).keys()
##             used_ids = {}.fromkeys(used_ids).keys()
##             missing_ids = {}.fromkeys(missing_ids).keys()
##             nodeid2count = {}
##             index2count = {}
##             for (nid, index) in used_ids:
##                 nodeid2count[nid] = nodeid2count.get(nid, 0) + 1
##                 index2count[index] = index2count.get(index, 0) + 1
##             if used_ids and max(nodeid2count.values()) > 1:
##                 continue
...
##     # Recursively check if this set of datatype_names can be inputs to
##     # this network.
##     return _find_paths_by_datanodes_h(
##         network, user_attributes, 0, data_nodes, node2ids)
...
def find_nodes_by_datatype(network, datatype_name):
    # Return a list of node_ids that match this datatype.
    node_ids = []
    for (node_id, node) in enumerate(network.nodes):
        if not isinstance(node, DataNode):
            continue
        if node.datatype.name == datatype_name:
            node_ids.append(node_id)
    return node_ids


def is_start_node(network, node_id):
    prev_ids = _backchain_to_ids(network, node_id)
    return not prev_ids
...
def _make_backchain_dict_h(network):
...
def _score_start_nodes(network, user_data, good_ids=None):
    ...
    results = []
    for node_id, next_ids in network.iterate(node_class=DataNode):
        if good_ids is not None and node_id not in good_ids:
            continue
