...
def send_query(handle, query):
    ...
    if i == 3:   # if variable assignment, find more solutions
        ...
        i = handle.expect(['true', 'false', 'ERROR', '[#\?]', ';'],
                          timeout=waitingtime)
        #if there is only one variable,will find 'false',
    ...
    while i == 0 or i == 4:
        if i == 0:
            result = result + handle.before
            # to check if the result is "true.", if yes, no more solution,exit
            try:
                i = handle.expect(['[#\?]'], timeout=0.5)
                break
            # if TimeoutError, there are more solutions
            except pexpect.TIMEOUT:
                handle.send(';')
                i = handle.expect(['true', 'false', 'ERROR', '[#\?]', ';'],
                                  timeout=waitingtime)
        # if found ';', to see if this is the last solution,
        # if not, find more solutions
        if i == 4:
            result = result + handle.before + ';'
            try:
                i = handle.expect(['true', 'false', 'ERROR', '[#\?]', ';'],
    ...
    newresult = [x for x in result if x.strip()]
    if newresult == []:
        return newresult
    else:
...
def parse_solution(solution_str):
    ...
    for i in variable_line:
        ...
        for j in range(len(index_list)):
            variables[i[start_list[j]:index_list[j]].strip()] = i[
                index_list[j] + 1:end_list[j]].strip()
