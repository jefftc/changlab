...
class Module(AbstractModule):
    ...
    def run(
        ...
        for filename in result_files:
            ...
            name = x
            assert name not in name2geneid2count
            
            x = _read_htseq_count_file(filename)
            name2geneid2count[name] = x
        assert name2geneid2count, "No samples"

        # Assemble into a gene expression matrix.
        all_names = sorted(name2geneid2count)
        all_genes = {}
        for n in all_names:
            all_genes.update(name2geneid2count[n])
        ...
        for gene_id in all_genes:
            counts = [
                name2geneid2count.get(n, {}).get(gene_id, 0)
                for n in all_names]
            x = [gene_id] + counts
            matrix.append(x)
