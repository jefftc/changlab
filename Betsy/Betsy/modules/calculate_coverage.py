...
class Module(AbstractModule):
    ...
    def run(
        ...
        if features_bed:
            cmds = []
            for x in jobs:
                sample, bam_filename, genomecov_filename, \
                        histo_datafile, histo_plotfile, histo_prismfile, \
                        intervallist_file, cov_filename, targetcov_filename, \
                        log_filename = x
                args = intervallist_file, features_bed, bam_filename
        ...
        if features_bed:
            ...
            for x in jobs:
                ...
                x = _make_calculatehsmetrics_command(
                    intervallist_file, bam_filename, cov_filename,
                    targetcov_filename, ref.fasta_file_full, log_filename)
                commands.append(x)
        ...
        for x in jobs:
            sample, bam_filename, genomecov_filename, \
                    ...
                    intervallist_file, cov_filename, targetcov_filename, \
                    log_filename = x
            _make_histo_file(genomecov_filename, histo_datafile)
...
def _run_genomecov(jobs, reference_file, num_cores):
    ...
    for x in jobs:
        ...
        x = ngslib.make_bedtools_genomecov_command(
            bam_filename, reference_file, genomecov_filename)
        commands.append(x)
    parallel.pshell(commands, max_procs=num_cores)
...
def _summarize_average_read_depth(jobs, min_coverage, summary_file):
    ...
    for x in jobs:
        ...
        if not os.path.exists(cov_filename):
            continue
        cov_data = _read_coverage_file(cov_filename)
        assert "MEAN_TARGET_COVERAGE" in cov_data
        mean_target_coverage[sample] = float(cov_data["MEAN_TARGET_COVERAGE"])
