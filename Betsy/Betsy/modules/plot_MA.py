...
def run(parameters,objects,pipeline):
    ...
    R('boxplot(x.read[,1], main="Before Normalization",xlab="samples", ylab="ratio")')
    R('dev.off()')
    jmath.R_equals('"' + os.path.join(outfile,'after_boxplot.png'+'"'),'filename')
    R('bitmap(file=filename,type="png256")')
    R('boxplot(x.norm[,1], main="After Normalization",xlab="samples", ylab="ratio")')
    R('dev.off()')
    jmath.R_equals('"' + os.path.join(outfile,'before_plot.png'+'"'),'filename')
    R('bitmap(file=filename,type="png256")')
    R('plot(x.read[,1], main="Before Normalization",xlab="samples", ylab="ratio")')
    R('dev.off()')
    jmath.R_equals('"' + os.path.join(outfile,'after_plot.png'+'"'),'filename')
    R('bitmap(file=filename,type="png256")')
    R('plot(x.norm[,1], main="After Normalization",xlab="samples", ylab="ratio")')
    R('dev.off()')
