...
%%class_label_file(+[X],[status,created], -Modules)
%make class_label_file from the signal file with the same content.
/*class_label_file([contents,[X],status,created], Modules):- 
    not(class_label_file([contents,[X],status,given],[])),
    signal_raw([contents,[X],status,created],Past_Modules),
...
/*class_label_file([contents,Contents,status,joined],Modules):-
    ...
    class_label_file([contents,C1,status,_],Past_Modules_1),
    class_label_file([contents,C2,status,_],Past_Modules_2),
    append(C1,C2,C3),
    C3=Contents,
...
%label file 
/*class_label_file([contents,Contents,status,split],Modules):-
    length(Contents,N),N>0,
    class_label_file([contents,C,status,given],Past_Modules),
