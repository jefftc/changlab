...
def run(parameters,objects,pipeline):
    ...
    if chipname not in ['HumanHT-12','MouseRef-8','HumanHT-12_control','MouseRef-8_control']:
        ...
        #annot_illumina_control(slice_BIN,chipfilename,single_object.identifier,outfile)
...
def annot_illumina_control(slice_BIN,annot_file,filename,outfile):
    annot_file = module_utils.gunzip(annot_file)
    f = file(annot_file,'r')
    text = f.readlines()
    start = text.index('[Controls]\n')
    end = text.index('[Columns]\n')
    f=file('annot.txt','w')
    for i in range(start+1,end):
        f.write(text[i])
    f.close()
    command2 = ['python',slice_BIN,'--add_row_annot',
                "annot.txt,Reporter_Group_Name",
                filename]
    f=file(outfile,'w')
    process = subprocess.Popen(command2,shell=False,
                                stdout=f,
