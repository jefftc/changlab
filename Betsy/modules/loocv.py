...
def run(parameters,objects,pipeline):
    ...
    for i in range(M.ncol()):
        ...
        if parameters['format'] == 'pcl':
            f_out = file(train_file,'w')
            arrayio.pcl_format.write(M_train,f_out)
            f_out.close()
            f_out = file(test_file,'w')
            arrayio.pcl_format.write(M_test,f_out)
            f_out.close()
        ...
        train_object = rule_engine.DataObject('signal_file',train_attributes,train_file)
        test_object = rule_engine.DataObject('signal_file',test_attributes,test_file)
        train_clfobject = rule_engine.DataObject('class_label_file',train_attributes,train_label)
        test_attributes.append('given')
        test_clfobject = rule_engine.DataObject('class_label_file',test_attributes,test_label)
