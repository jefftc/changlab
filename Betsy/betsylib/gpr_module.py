...
def check_gpr(fileloc):
    ...
    entry=['Type','DateTime','Settings','GalFile','PixelSize','Wavelengths','ImageFiles',
       'NormalizationMethod','NormalizationFactors','JpegImage','StdDev','RatioFormulations',
       'Barcode','BackgroundSubtraction','ImageOrigin','JpegOrigin','Creator','Scanner',
       'FocusPosition','Temperature','LinesAveraged','Comment','PMTGain','ScanPower',
       'LaserPower','LaserOnTime','Filters','ScanRegion','Supplier']
    ...
    if not text[0].startswith('ATF'):#'the gpr requires a ATF number'
        return False
    a=text[1].split()
    if not(len(a)==2 and a[0].isdigit() and a[1].isdigit()):#'should be interger'
        return False
    i=-1
    startline=-1
    while i<len(text):
...
def extract_gpr(fileloc,keep):
    ...
    for i in range(startline,len(text)):
        ...
        logratio.append(x)
    logratio[0]=samplename #replace the samplename to the "Log Ratio (635/532)"
