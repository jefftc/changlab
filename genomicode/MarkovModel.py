...
def _numpy_log_min_num(x):
    return numpy.log(x + VERY_SMALL_NUMBER)
...
def find_states(markov_model, output):
    ...
    # These three lines take about 33% of the time in this function
    # (not including _viterbi).  Probably hard to get it to go much
    # faster, though.
    lp_initial = _numpy_log_min_num(mm.p_initial)
    lp_transition = _numpy_log_min_num(mm.p_transition)
    lp_emission = _numpy_log_min_num(mm.p_emission)
...
def _max_sum_and_index(array1, array2):
    sums = array1 + array2
    index = sums.argmax()
    return sums[index], index


def _viterbi_h(scores, backtrace, N, T, output, lp_trans_j, lp_emission):
    # scores and backtrace are modified in place.
    for t in range(1, T):
        k = output[t]
        # Make a new numpy.array to ensure that it is contiguous.
        scores_prev = numpy.array(scores[:,t-1])
        for j in range(N):
            # Find the most likely place it came from.
            score, index = _max_sum_and_index(scores_prev, lp_trans_j[j])
            scores[j,t] = score + lp_emission[j,k]
            backtrace[j][t] = [index]
...
def _viterbi(N, lp_initial, lp_transition, lp_emission, output):
    ...
    # Optimization.  Pull the slicing out of the inner loop.
    lp_trans_j = [None] * N
    for j in range(N):
        # Make a new numpy.array to ensure that it is contiguous.
        lp_trans_j[j] = numpy.array(lp_transition[:,j])
