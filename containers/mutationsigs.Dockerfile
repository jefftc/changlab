...
# Install a few dependencies at a time, so don't have to redo every
# time something changes.
RUN Rscript -e "install.packages('Rcpp')"
RUN Rscript -e "install.packages(c('openssl', 'XML', 'curl'))"
RUN Rscript -e "install.packages(c('stringr', 'httr', 'magrittr'))"
...
RUN Rscript -e "BiocManager::install(c('BSgenome', 'BSgenome.Hsapiens.UCSC.hg19'))"
RUN Rscript -e "BiocManager::install(c('GenomeInfoDb'))"
...
RUN Rscript -e "install.packages(c('callr', 'withr', 'pkgbuild'))"
...
RUN apt-get install -y --no-install-recommends \
    git
RUN cd /usr/local/src && \
    git clone https://github.com/parklab/SigMA.git

RUN Rscript -e "install.packages(c('xml2', 'yaml', 'markdown', 'htmltools'))"
RUN Rscript -e "install.packages(c('knitr', 'highr'))"
RUN Rscript -e "install.packages(c('xopen', 'xfun'))"
RUN cd /usr/local/src/SigMA && \
    Rscript install.R
